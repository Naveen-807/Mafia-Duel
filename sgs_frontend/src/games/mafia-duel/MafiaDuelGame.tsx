...
export function MafiaDuelGame({ userAddress, onBack, onGameComplete }: MafiaDuelGameProps) {
  ...
  // Reset ZK proof state when phase or round changes
  useEffect(() => { setZkProof(null); }, [game?.phase, game?.day]);
  ...
  async function handleAction(target: number) {
    // ── ZK Step 1: Poseidon commit (non-fatal — voting always proceeds) ─
    // If the ZK library throws for any reason, we log it and carry on.
    // The submission to the chain is the authoritative action.
    if (game && sid !== null) {
      try {
        const proof = zkCommit(target, sid, game.day, game.phase);
        setZkProof(proof);
      } catch (zkErr) {
        console.warn('[ZK] Poseidon commit failed (non-fatal):', zkErr);
      }
    }
    // ── ZK Step 2: submit to contract (always runs) ────────────────
  ...
  // ── ZK Proof Panel ─────────────────────────────────────────────────────
  // Shows the Poseidon commitment, nonce, and live verification status.
  function ZkProofPanel({ proof, slotNames }: { proof: ZkCommitment; slotNames: string[] }) {
    const verified = zkVerify(proof.commitment, proof.target, proof.nonce);
    ...
    return (
      <div style={{ background:'rgba(0,255,150,0.05)', border:'1px solid rgba(0,255,150,0.2)', borderRadius:10, padding:'11px 14px', marginTop:12 }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', fontSize:11, color:'#475569' }}>
          <span>Action: <b style={{ color:'#e2e8f0' }}>{tLabel}</b></span>
          <span style={{ color: verified ? '#4ade80' : '#f87171', fontWeight:700 }}>
            {verified ? '✅ Verified' : '❌ Mismatch'}
          </span>
        </div>
        <div style={{ fontSize:10, color:'#334155', marginTop:6, lineHeight:1.6 }}>
          C = Poseidon({proof.target === 4294967295 ? 'PASS' : proof.target}, secret_nonce) &nbsp;|&nbsp;
          Nonce: {shortCommitment('0x' + proof.nonce.toString(16))}
        </div>
      </div>
    );
  }
  ...
  function LastEvent() {
    ...
    // ZK verification panel — show after last round
    const prevPhase = game.phase === PHASE_DAY ? PHASE_NIGHT : PHASE_DAY;
    const prevRound = game.phase === PHASE_NIGHT ? Math.max(0, game.day - 1) : game.day;
    const storedProof = getStoredCommitment(sid!, prevRound, prevPhase);
    if (storedProof) {
      const verified = zkVerify(storedProof.commitment, storedProof.target, BigInt(storedProof.nonce));
      const slotNames = game.slots.map((sl, i) => slotName(sl, i));
      lines.push({
        icon: '🔐',
        text: `ZK reveal: your committed action was "${targetLabel(storedProof.target, slotNames)}" — Poseidon proof ${verified ? '✅ verified' : '⚠️ unverifiable'}.`,
      });
    }
