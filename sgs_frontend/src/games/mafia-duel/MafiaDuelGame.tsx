...
export function MafiaDuelGame({ userAddress, onBack, onGameComplete }: MafiaDuelGameProps) {
  ...
  // ── NIGHT COMMIT PHASE ────────────────────────────────────────────────────
  // Players choose their target and submit a SHA-256 commitment.
  // The plaintext target is NOT sent onchain yet — only the hash.
  if (game.phase === PHASE_NIGHT_COMMIT) {
    ...
    return (
      <div style={C.page}>
        <div style={C.wrap}>
          <WalletBar />
          <PageHeader phase={PHASE_NIGHT_COMMIT} />
          <Alerts />
          <LastEvent />
          ...
          <div style={{ background:'rgba(99,102,241,0.08)', border:'1px solid rgba(99,102,241,0.25)', borderRadius:9, padding:'10px 13px', marginBottom:12, fontSize:12, color:'#a5b4fc' }}>
            🔐 <b>ZK Night:</b> Your choice is hashed with SHA-256 before being sent to the contract.
            The plaintext target stays hidden until reveal. Contract verifies the hash onchain.
          </div>
          ...
          <div style={C.card()}>
            {!inGame ? (
              ...
              <div style={{ padding:'10px 0' }}>
                <div style={{ textAlign:'center' }}>
                  ...
                  <div style={{ fontWeight:700, color:'#4ade80', fontSize:15 }}>Commitment submitted!</div>
                  <div style={{ fontSize:12, color:'#64748b', marginTop:4 }}>
                    Your target is locked in — hidden until all players commit.
                    {pending > 0 ? ` Waiting for ${pending} more…` : ''}
                  </div>
                </div>
                {zkProof && (
                  <ZkProofPanel
                    proof={zkProof}
                    slotNames={game.slots.map((sl, i) => slotName(sl, i))}
                  />
                )}
              </div>
              ...
              <div style={{ textAlign:'center', padding:'8px 0' }}>
                <div style={{ fontSize:26, marginBottom:5 }}>😴</div>
                <div style={{ fontWeight:600, color:'#94a3b8', fontSize:14 }}>Auto-committing pass for you…</div>
                <div style={{ fontSize:12, color:'#475569', marginTop:4 }}>Villagers sleep — a sha256 pass commitment is submitted automatically.</div>
              </div>
            ) : (
              <>
                {me.role === ROLE_MAFIA && (
                  <>
                    <div style={{ color:'#f87171', fontWeight:600, fontSize:13, marginBottom:12 }}>🔪 Kill — choose your target (commitment only):</div>
                    {townAlive.length === 0
                      ? <div style={{ color:'#475569', fontSize:12 }}>No Town players alive.</div>
                      : townAlive.map(({ sl, i }) => (
                          <button key={i} style={C.btnRow('rgba(220,38,38,0.2)')} disabled={loading} onClick={() => handleNightCommit(i)}>
                ...
                {me.role === ROLE_DOCTOR && (
                  <>
                    <div style={{ color:'#4ade80', fontWeight:600, fontSize:13, marginBottom:12 }}>💊 Protect — choose who to save (commitment only):</div>
                    {alive.map(({ sl, i }) => (
                      <button key={i} style={C.btnRow('rgba(34,197,94,0.18)')} disabled={loading} onClick={() => handleNightCommit(i)}>
                        {slotIcon(sl)} {slotName(sl, i)}
                      </button>
                    ))}
                  </>
                ...
                {me.role === ROLE_SHERIFF && (
                  <>
                    <div style={{ color:'#facc15', fontWeight:600, fontSize:13, marginBottom:12 }}>⭐ Investigate — choose a player to investigate (commitment only):</div>
                    {alive.filter(({ i }) => i !== myIdx).map(({ sl, i }) => (
                      <button key={i} style={C.btnRow('rgba(234,179,8,0.18)')} disabled={loading} onClick={() => handleNightCommit(i)}>
                        {slotIcon(sl)} {slotName(sl, i)}
                      </button>
                    ))}
                  </>
            ...
            <div style={{ fontSize:12, color:'#475569', marginBottom:9 }}>
              {pending > 0
                ? `${pending} human${pending > 1 ? 's' : ''} still committing. Resolve advances automatically after all commit.`
                : 'All committed — phase will advance to reveal.'}
            </div>
          </div>
  ...
  // ── NIGHT REVEAL PHASE ────────────────────────────────────────────────────
  // All players have committed. Now they reveal — contract verifies sha256 onchain.
  if (game.phase === PHASE_NIGHT_REVEAL) {
    ...
    return (
      <div style={C.page}>
        <div style={C.wrap}>
          <WalletBar />
          <PageHeader phase={PHASE_NIGHT_REVEAL} />
          <Alerts />
          ...
          {me && rm && (
            <div style={{ ...C.card(rm.color), marginBottom:12 }}>
              <div style={{ display:'flex', alignItems:'center', gap:13 }}>
                ...
                <div>
                  <div style={{ fontWeight:800, fontSize:17, color:rm.color }}>{rm.label}</div>
                  <div style={{ fontSize:12, color:'#94a3b8', marginTop:2 }}>All players committed — time to reveal.</div>
                </div>
              </div>
            </div>
          )}
          ...
          <div style={{ background:'rgba(99,102,241,0.08)', border:'1px solid rgba(99,102,241,0.25)', borderRadius:9, padding:'10px 13px', marginBottom:12, fontSize:12, color:'#a5b4fc' }}>
            ...
            to your stored commitment. A mismatch → InvalidReveal (#12) — you cannot change your target.
          </div>
          ...
          <div style={C.card()}>
            {!inGame ? (
              ...
              <div style={{ textAlign:'center', padding:'10px 0' }}>
                <div style={{ fontSize:26, marginBottom:5 }}>✅</div>
                <div style={{ fontWeight:700, color:'#4ade80', fontSize:15 }}>Action revealed!</div>
                <div style={{ fontSize:12, color:'#64748b', marginTop:4 }}>
                  Contract verified your commitment onchain.
                  {pending > 0 ? ` Waiting for ${pending} more…` : ' All revealed — resolve now.'}
                </div>
              </div>
            ) : me?.role === ROLE_VILLAGER ? (
              <div style={{ textAlign:'center', padding:'8px 0' }}>
                <div style={{ fontSize:26, marginBottom:5 }}>😴</div>
                <div style={{ fontWeight:600, color:'#94a3b8', fontSize:14 }}>Auto-revealing your pass…</div>
              </div>
            ) : stored ? (
              <div style={{ padding:'10px 0' }}>
                {zkProof && (
                ...
                <div style={{ fontSize:12, color:'#94a3b8', marginTop:12, marginBottom:8 }}>
                  You committed to: <b style={{ color:'#e2e8f0' }}>{targetLabel(stored.target, game.slots.map((sl, i) => slotName(sl, i)))}</b>
                </div>
                <button
                  style={C.btn('linear-gradient(135deg,#6366f1,#4f46e5)')}
                  disabled={loading}
                  onClick={handleNightReveal}
                >
                  {loading ? '⏳ Revealing…' : '🔓 Reveal Action (Verified Onchain)'}
                </button>
              </div>
            ) : (
              <div style={{ color:'#f87171', textAlign:'center', padding:'8px 0', fontSize:13 }}>
                ⚠️ No commitment found for this round. Refresh the page and try again.
              </div>
            )}
            ...
            <div style={{ fontSize:12, color:'#475569', marginBottom:9 }}>
              {pending > 0
                ? `${pending} human${pending > 1 ? 's' : ''} yet to reveal. Resolve after all reveal:`
          ...
          <div style={C.card('#b45309')}>
            {!inGame ? (
              ...
              <>
                ...
                {alive.filter(({ i }) => i !== myIdx).map(({ sl, i }) => (
                  <button key={i} style={C.btnRow('rgba(180,83,9,0.25)')} disabled={loading} onClick={() => handleDayAction(i)}>
                    {slotIcon(sl)} {slotName(sl, i)}
                  </button>
                ))}
                <button style={{ ...C.btnRow('rgba(100,116,139,0.18)'), color:'#94a3b8', fontStyle:'italic' }} disabled={loading} onClick={() => handleDayAction(PASS_TARGET)}>
                  🤷 Abstain (no vote)
                </button>
              </>
            )}
